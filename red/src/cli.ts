...
import { automqWorkflow } from "./workflow.ts";

export const lifecycleCommands = ["build", "create", "delete", "validate"];

export const usage =
  "Usage: red <build|create|delete|validate> [-f|--file colors.yml] [--dry-run]\n" +
  "\n" +
...
export async function run(...args: string[]): Promise<Opts> {
  ...
  if (lifecycleCommands.includes(command)) {
    return runCli(automqWorkflow, withFile);
  }
...
export async function exec(args: string[] = Bun.argv.slice(2)): Promise<never> {
  if (lifecycleCommands.includes(args[0] ?? "")) {
    return execCli(automqWorkflow, defaultArgs(args));
  }
