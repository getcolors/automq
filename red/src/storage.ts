...
export function specs(opts: Opts): Spec[] {
  return [{template:{name:"tools/storage/main.tf", content:mainTf}, target:directory(opts)+"/main.tf", data:{...opts,"automq-storage-gcs":opts["automq-storage-provider"] === "gcs"}, opts:PRESERVE_JINJA_DELIMITERS}];
}
