...
[ -f "$tmp/project/.colors/automq-fixture/automq-infrastructure/shared/shared.tf.json" ] || fail 'copied green payload rendered nothing'
grep -q '"vultr_vpc"' "$tmp/project/.colors/automq-fixture/automq-infrastructure/shared/shared.tf.json" || fail 'rendered compute has no VPC'
...
grep -q '"vultr_vpc"' "$tmp/red-project/.colors/automq-fixture/automq-infrastructure/shared/shared.tf.json" || fail 'red rendered compute has no VPC'
...
grep -q '"vultr_vpc"' "$tmp/blue-project/.colors/automq-fixture/automq-infrastructure/shared/shared.tf.json" || fail 'blue rendered compute has no VPC'
...
# deployment, not here — which is how the red payload sat at 759eb03 for one
# release after the package moved to b1628b7.
