...
BOOTSTRAP="${1:-10.40.0.10:9094,10.40.0.11:9094,10.40.0.12:9094}"
...
docker exec automq "$KAFKA/kafka-configs.sh" --bootstrap-server "$BOOTSTRAP" \
  ...
  --entity-type users --entity-name "automq" >/dev/null
...
sasl.jaas.config=org.apache.kafka.common.security.scram.ScramLoginModule required username="automq" password="${new}";
EOF
