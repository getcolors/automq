...
sasl.jaas.config=org.apache.kafka.common.security.scram.ScramLoginModule required username="automq-admin" password="${AUTOMQ_ADMIN_PASSWORD}";
EOF
chmod 0600 "$tmp"
...
security.protocol=SASL_PLAINTEXT
sasl.mechanism=SCRAM-SHA-512
sasl.jaas.config=org.apache.kafka.common.security.scram.ScramLoginModule required username="automq" password="${AUTOMQ_CLIENT_PASSWORD}";
EOF
chmod 0600 "$tmp"
