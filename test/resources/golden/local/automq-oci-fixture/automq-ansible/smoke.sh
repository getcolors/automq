...
k kafka-topics.sh --bootstrap-server "$BOOTSTRAP" --command-config "$ADMIN" \
  ...
  --partitions 6 --replication-factor 1 >/dev/null
...
for role in data ops; do
  bucket=$([ "$role" = data ] && echo "automq-oci-fixture-data-example" || echo "automq-oci-fixture-ops-example")
  n=$(python3 - "$bucket" <<'PY'
import os, sys, boto3
from botocore.config import Config
s3 = boto3.client("s3", endpoint_url="https://example.compat.objectstorage.eu-frankfurt-1.oraclecloud.com", region_name="eu-frankfurt-1",
...
sasl.jaas.config=org.apache.kafka.common.security.scram.ScramLoginModule required username="automq-admin" password="definitely-not-the-password";
EOF
...
gate "the client principal cannot create a topic outside colors-"
