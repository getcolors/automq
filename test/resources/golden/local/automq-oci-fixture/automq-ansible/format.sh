...
if [ -f "$META" ]; then
  ...
  if [ "$have_cluster" != "bzC0Rv_0SBuO0ykEvTn1sg" ]; then
    echo "FATAL: $META belongs to cluster $have_cluster, not bzC0Rv_0SBuO0ykEvTn1sg." >&2
...
args=(--cluster-id "bzC0Rv_0SBuO0ykEvTn1sg" --config "$CONTAINER_CONFIG")
...
if [ "$GENESIS" = "true" ]; then
  ...
  args+=(--add-scram "SCRAM-SHA-512=[name=automq-admin,salt=${AUTOMQ_ADMIN_SALT},saltedpassword=${AUTOMQ_ADMIN_SALTED},iterations=${it}]")
  args+=(--add-scram "SCRAM-SHA-512=[name=automq-broker,salt=${AUTOMQ_BROKER_SALT},saltedpassword=${AUTOMQ_BROKER_SALTED},iterations=${it}]")
  args+=(--add-scram "SCRAM-SHA-512=[name=automq,salt=${AUTOMQ_CLIENT_SALT},saltedpassword=${AUTOMQ_CLIENT_SALTED},iterations=${it}]")
