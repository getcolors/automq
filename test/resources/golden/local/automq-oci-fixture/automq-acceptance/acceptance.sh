...
read_offsets() {
  on 0 "sudo docker exec -e KAFKA_HEAP_OPTS=-Xmx256m automq /opt/automq/kafka/bin/kafka-consumer-groups.sh \
    --bootstrap-server 10.0.0.10:9094,10.0.0.11:9094,10.0.0.12:9094 --command-config /etc/automq/admin.properties \
...
if [ -z "$victim" ]; then
  ...
  while [ "$SECONDS" -lt "$DEADLINE" ]; do
    ...
    brokers=$(on 0 "sudo docker exec -e KAFKA_HEAP_OPTS=-Xmx256m automq /opt/automq/kafka/bin/kafka-broker-api-versions.sh \
      --bootstrap-server 10.0.0.10:9094,10.0.0.11:9094,10.0.0.12:9094 --command-config /etc/automq/admin.properties" 2>/dev/null | grep -c 'id: ')
...
if perf=$(on 0 "sudo docker exec -e KAFKA_HEAP_OPTS=-Xmx256m automq /opt/automq/kafka/bin/kafka-producer-perf-test.sh \
  ...
  --producer-props bootstrap.servers=10.0.0.10:9094,10.0.0.11:9094,10.0.0.12:9094" 2>/dev/null | tail -1)
then
