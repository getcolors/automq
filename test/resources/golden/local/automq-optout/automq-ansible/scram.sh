...
EOF
  ...
  docker exec -e KAFKA_HEAP_OPTS=-Xmx256m automq rm -f /tmp/probe.properties >/dev/null 2>&1
  rm -f "$f"
  return $rc
}
...
if ! probe "automq-admin" "$AUTOMQ_ADMIN_PASSWORD"; then
  cat >&2 <<EOF
FATAL: the admin principal automq-admin cannot authenticate.
...
bootstrap records. Check:
  ...
  docker exec -e KAFKA_HEAP_OPTS=-Xmx256m automq $KAFKA/kafka-dump-log.sh --cluster-metadata-decoder \\
...
changed=0
for pair in "automq:$AUTOMQ_CLIENT_PASSWORD" "automq-broker:$AUTOMQ_BROKER_PASSWORD"; do
  user="${pair%%:*}"; pass="${pair#*:}"
