...
if [ -f "$META" ]; then
  ...
  if [ "$have_cluster" != "Vn4JS1TOSW6VmtoBbClErQ" ]; then
    echo "FATAL: $META belongs to cluster $have_cluster, not Vn4JS1TOSW6VmtoBbClErQ." >&2
...
args=(--cluster-id "Vn4JS1TOSW6VmtoBbClErQ" --config "$CONTAINER_CONFIG")
